...
namespace ColbyO.VNTG.ColorPalette.Editor
{
    public static class LUTBaker
    {
        ...
        public static void BakePaletteTo3DLUT(List<Color> palette, PaletteDistanceMetric metric, ref Color32[] rawColors, ref Texture3D lut)
        {
            ...
            if (lut == null || lut.width != LUT_DIM || lut.height != LUT_DIM || lut.depth != LUT_DIM)
            {
                lut = new Texture3D(LUT_DIM, LUT_DIM, LUT_DIM, TextureFormat.RGBA32, false);
            }
            ...
            try
            {
                int totalSteps = LUT_DIM * LUT_DIM;

                for (int r = 0; r < LUT_DIM; r++)
                {
                    for (int g = 0; g < LUT_DIM; g++)
                    {
                        int currentStep = (r * LUT_DIM) + g;
                        float progress = (float)currentStep / totalSteps;

                        EditorUtility.DisplayProgressBar(
                            "Baking LUT",
                            $"Processing color space... ({currentStep}/{totalSteps})",
                            progress
                        );
        ...
        public static void Rebake(Palette palette)
        {
            if (palette == null) return;

            bool isNewTexture = (palette.LUT == null);

            BakePaletteTo3DLUT(palette.colors, palette.distanceMetric, ref palette.RawLUT, ref palette.LUT);
            if (palette.LUT == null) return;

            palette.LUT.name = "Baked_LUT";
            palette.LUT.hideFlags = HideFlags.None;

            if (isNewTexture)
            {
                string parentPath = AssetDatabase.GetAssetPath(palette);
                if (!string.IsNullOrEmpty(parentPath))
                {
                    EditorApplication.delayCall += () =>
                    {
                        if (palette == null || palette.LUT == null || string.IsNullOrEmpty(parentPath)) return;

                        AssetDatabase.AddObjectToAsset(palette.LUT, palette);
            ...
            {
                EditorUtility.SetDirty(palette);
                EditorUtility.SetDirty(palette.LUT);
                AssetDatabase.SaveAssets();
            }
        }
    }
